...
import { SocialNetworkList, SocialNetworkListModifier } from "@/features/SocialNetwork";
...
export const Footer: FC<FooterProps> = memo(({ className }) => {
	...
	const DisplayContactSection = useCallback(() => {
		if (!isTablet) {
			return null;
		}
		...
		return (
			<section className={cn(cls.ContactSection, {}, [cls.Footer__contactSection])}>
				<Container className={cls.ContactSection__container}>
					<Address
						className={cls.ContactSection__address}
						slotForFeature={(
							<SocialNetworkList
								modifier={SocialNetworkListModifier.FOOTER}
							/>
						)}
					/>
				</Container>
					...
					<div className={cls.MainSection__main}>
						{!isTablet && (
							<Address
								...
								slotForFeature={<SocialNetworkList modifier={SocialNetworkListModifier.FOOTER} />}
